  ...
  onMount(() => {
    const DemoApp = {
      ...
      init(options) {
        ...
        Telegram.WebApp.BackButton.onClick(() => navigation.back());
      },
      expand() {
...
<div class="container">
  <section id="top_sect" class="second">
    <button id="main_btn" on:click="{DemoApp.sendMessage('')}"
      >Send «Hello, World!»</button
    >
    <button id="with_webview_btn" on:click="{DemoApp.sendMessage('', true)}"
      >Send «Hello, World!» with inline webview button</button
    >
    <button id="data_btn" on:click="{DemoApp.sendTime(true)}"
      >Send current time to bot (x10)</button
    >
    <button on:click="{DemoApp.expand()}">Expand Webview</button>
    <button on:click="{DemoApp.toggleMainButton()}">Hide Main Button</button>
    <input
      type="text"
